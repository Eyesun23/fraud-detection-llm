...
def load_corpus(
    ...
    if full_corpus:
        return get_or_fetch_enron_corpus(save_path=save_corpus_path)
...
def download_and_process_gemini_batch(job_name, original_emails, save_path, client):
    """
    ...
    while True:
        ...
        if state == 'JOB_STATE_SUCCEEDED':
            print("\n[Batch API] Job completed successfully.")
            break
        elif state in ['JOB_STATE_FAILED', 'JOB_STATE_CANCELLED']:
            raise RuntimeError(f"Batch job ended with state: {state}. Error: {batch_job.error}")
    ...
    print(f"Successfully saved {len(raw_lines)} results to {output_filename}")
    ...
    for line in raw_lines:
        ...
        try:
            ...
            e["category"] = r.get("primary_wrongdoing", "unknown")
            ...
            e["llm_total"] = 0
            e["category"] = "error"
            e["rationale"] = f"Parse Error: {str(ex)}"
...
def create_batch_request(emails, save_path):
    file_path = os.path.join(save_path, "batch-requests.jsonl")
    ...
    print(f"Created {len(emails)} batch-requests")
    return file_path


def run_pipeline(emails: list[dict], save_path="") -> tuple[pd.DataFrame, list[dict]]:
    """
    ...
    file_path = create_batch_request(filtered_emails, save_path)
...
if __name__ == "__main__":
    ...
    df, top5 = run_pipeline(emails, save_path=SAVE_PATH)
